...
Скорее всего подойдёт для солничных инверторов SILA.
Небольшое количество данных взято из [этой](https://nsk-electro.ru/blog-news/monitoring-invertor-sila-esp8266) статьи.
...
* У инвертора SmartWatt ECO 7.2К есть второй канал для волничных панелей. Данные по состоянию второго канала не обрабатываются.
* Написать инструкцию по установке экспортера через Docker
* Написать инструкцию по настройке ser2net + socat чтобы собирать данные через один экспортер с больщого количества инверторов.